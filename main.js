...
const Pagination = ({ data, itemsPerPage }) => {
  ...
  const createNavigationButton = (text, pageNumber, isDisabled) => {
    ...
    button.disabled = isDisabled; // button.disabled = (i === currentPage); 같은 작동.
  ...
  const createPagination = () => {
    ...
    // 현재 페이지의 4페이지 전부터 보여주되, 1페이지보다 작아질 순 없다"는 의미
    ...
    for (let i = startPage; i <= endPage; i++) {
      paginationContainer.appendChild(
        createNavigationButton(i, i, i === currentPage)
      );
    }
    // 마지막 페이지로 이동하는 버튼 추가
    // totalPages가 20이고, currentPage가 15일 때
    // 1. 버튼에 표시될 텍스트: ">>"
    // 2. 클릭시 이동할 페이지: 20 (마지막 페이지): totalpages
    // 3. currentPage === totalPages : 현재 페이지가 마지막 페이지인지: false (15 !== 20)
    // 다른예시) totalPages가 20이고, currentPage가 20일 때 (마지막 페이지)
    // 결과: <button class="page-btn" disabled>>></button>
    // currentPage === totalPages: 이미 마지막 페이지면 버튼 비활성화
    // 현재 마지막 페이지?: true (20 === 20)
    paginationContainer.appendChild(
...
const displayData = (data, currentPage, itemsPerPage) => {
  ...
  // 현재 페이지에 해당하는 데이터만 추출하여 화면에 표시
  data.slice(startIndex, endIndex).forEach((item) => {
    // 각 항목에 대해 리스트 아이템(li) 생성
    const li = document.createElement("li");
    // CSS 클래스 추가
    li.classList.add("content-item");

    // 각 아이템을 클릭 가능한 링크로 만들기
    const link = document.createElement("a");

    // URL에는 특수문자나 공백 등을 직접 포함할 수 없음.
    // encodeURIComponent()는 이러한 특수문자들을 URL에서 사용 가능한 형식으로 변환
    // detail.html?item : detail.html - 페이지 이름
    // ? - 쿼리 파라미터의 시작을 나타내는 구분자 (이건 고정)
    // item - 파라미터 이름 (완전히 자유롭게 정할 수 있음)
    link.href = `detail.html?item=${encodeURIComponent(item)}`;
    // 아이템 텍스트 설정
    link.textContent = item;
    // link.target = "_blank"; // 새 창에서 열기

    li.appendChild(link);
    // 생성된 리스트 아이템을 리스트에 추가
    contentList.appendChild(li);
  });
};

// DOM이 완전히 로드된 후 실행되는 코드
// 모든 DOM 요소가 준비되면 자동으로 등록된 함수를 실행
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const sampleData = Array.from({ length: 200 }, (_, i) => `아이템 ${i + 1}`);
  ...
  // 현재 페이지의 데이터를 화면에 표시
  // 1단계: URL에서 페이지 번호 가져오기
  const currentPage =
    new URLSearchParams(window.location.search).get("page") || 1;
  // 예시: URL이 www.yoursite.com?page=3 이면 -> 3을 가져옴
  // URL이 www.yoursite.com 이면 (page가 없으면) -> 1을 사용

  // 2단계: 실제 데이터 표시하기
  displayData(
    sampleData, // 전체 데이터 배열
    parseInt(currentPage), // 현재 페이지 번호를 숫자로 변환
    itemsPerPage
  ); // 한 페이지당 표시할 개수
});

// parseInt는 JavaScript 내장 함수.
// 문자열을 숫자로 변환해주는 메서드
// parseInt("123") - 결과: 123 (숫자)
// parseInt("5") - 결과: 5 (숫자)
